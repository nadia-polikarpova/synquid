...
TIMEOUT = 3600                                                  # Timeout value (seconds) 
...
COMMON_OPTS = ['--print-stats',
               '--memoize'
               ]                                     # Options to use for all benchmarks
INCREMENTAL_OFF_OPT = ['--incremental=0', '--consistency=0']    # Option to disable local liquid type checking features
...
ALL_BENCHMARKS = [
    ...
    # BenchmarkGroup("RBT", ['-m=1', '-a=2'], [
        # Benchmark('RBT-BalanceL', 'balance left', '', ['-u', '-a=2']),
        # Benchmark('RBT-BalanceR', 'balance right', '', ['-u', '-a=2']),
    ...
    BenchmarkGroup("User", [], [        
        ...
        Benchmark('RunLen-Encode', 'run length encoder', '0, 1, inc, dec, $\\leq$, $\\neq$'),
        Benchmark('RunLen-Decode', 'run length decoder', '0, 1, inc, dec, $\\leq$, $\\neq$', ['-a=4']),
        Benchmark('AST-DesugarBool', 'desugar bool into int', '0, 1'),
        Benchmark('AST-DesugarBool2', 'desugar bool and int into int', '0, 1'),        
        # Benchmark('AST-NegationNF', 'negation normal form', 'true, false'),
        # Benchmark('AST-NegationNF2', 'NNF w/literal elimination', 'true, false')
        ])
]
...
class SynthesisResult:
    def __init__(self, name, time, goal_count, code_size, spec_size, measure_count):
        ...
        self.variant_outcome = {                # Synthesis outcome for Synquid variants:
                                'nrt': '',        # round-trip checking disabled
                             }        
...
def run_benchmark(name, opts, default_opts):
    ...
    with open(LOGFILE, 'a+') as logfile:
      ...
      return_code = subprocess.call([SYNQUID_CMD] + COMMON_OPTS + opts + [name + '.sq'], stdout=logfile, stderr=logfile)
      end = time.time()
      ...
      # Run with no round-trip type checking:
      if (not (cl_opts.medium or cl_opts.small)):
        run_version(name, 'nrt', opts + INCREMENTAL_OFF_OPT, logfile)
